...
from ngio_tables05.common import SAMPLE_FEATURE_TABLE_DF, compare_dataframes, TableCheckResult, save_results_to_csv, SAMPLE_MASKING_ROI_TABLE_DF, SAMPLE_ROI_TABLE_DF
import zarr
import pandas as pd
...
def check_table(dir_path: Path,
    ...
    table_lib, table_backend = dir_path.as_posix().split("/")[-2:]
...
def ngio_table_create(args, zarr_format: int, current_lib):
    base = Path(args.dir).absolute()
    base.mkdir(parents=True, exist_ok=True)
    for backend in ["anndata", "json", "csv", "parquet"]:
        table_dir = base / f"{current_lib}" / f"{backend}"
